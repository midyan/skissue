...
// skissue CLI layout — commands → core | git | registry; git → core; registry → (none).
...
export function getLayer(filePath: string, srcDir: string): string | undefined {
  const srcPath = join(srcDir, "src");
  const rel = relative(srcPath, filePath);
  if (rel === "" || rel.startsWith("..")) return undefined;
  const firstSeg = rel.split(/[/\\]/).find((s) => s.length > 0);
  if (firstSeg === undefined) return undefined;
...
const isCLI = Boolean(process.argv[1]?.includes("validate-deps/hard/index.ts"));
