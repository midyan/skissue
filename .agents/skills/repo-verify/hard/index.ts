 ...
 * fallback chain (prefers `test:coverage` over `test`). Never part of check:all (see .no-auto-run).
 */
...
export interface HardSkillDiscovery {
  id: string;
  /** True when registry/runner.ts will execute this skill's hard/index.ts */
...
/**
 * Lists every `registry/<id>/hard/index.ts` and whether it participates in check:all.
 */
...
export function resolveVerifyPlan(root: string): VerifyPlan {
  ...
  const beforeTest = ["lint", "format:check"] as const;
  for (const name of beforeTest) {
    if (scripts?.[name]?.trim()) {
      steps.push({ label: `npm run ${name}`, argv: ["npm", "run", name] });
    }
  }

  const testScript = scripts?.["test:coverage"]?.trim()
    ? "test:coverage"
    : scripts?.test?.trim()
      ? "test"
      : undefined;
  if (testScript) {
    steps.push({ label: `npm run ${testScript}`, argv: ["npm", "run", testScript] });
  }

  const afterTest = ["check:all", "build"] as const;
  for (const name of afterTest) {
    if (scripts?.[name]?.trim()) {
      steps.push({ label: `npm run ${name}`, argv: ["npm", "run", name] });
    }
  }
...
function runPlan(root: string, plan: VerifyPlan, dryRun: boolean): boolean {
...
const isCLI = process.argv[1]?.includes("repo-verify/hard/index.ts") ?? false;

if (isCLI) {
  const { dryRun, discoverOnly } = parseArgs(process.argv.slice(2));
  const registryDir = join(ROOT, "registry");

  printDiscovery(registryDir);
