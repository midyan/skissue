...
function registryAutoHardSkillCount(root: string): number {
  const skillsDir = join(root, "registry");
  if (!fileExists(skillsDir)) return 0;
  try {
...
function registryHardSkillsHaveTests(root: string): boolean {
  const skillsDir = join(root, "registry");
  if (!fileExists(skillsDir)) return false;
  try {
...
const AUTOMATION_CRITERIA: AutomationCriteria[] = [
  {
    label: "Registry runner (registry/runner.ts)",
    check: (r) => fileExists(join(r, "registry", "runner.ts")),
  },
  {
    label: "At least 2 auto-run hard skill components",
    check: (r) => registryAutoHardSkillCount(r) >= 2,
  },
  {
  ...
  {
    label: "Registry hard skills have unit tests",
    check: (r) => registryHardSkillsHaveTests(r),
  },
];
...
const isCLI = process.argv[1]?.includes("report-harness-score/hard/index.ts") ?? false;
