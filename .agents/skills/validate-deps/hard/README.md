...
Validates dependency direction between top-level **`src/`** layers. The default graph in **`index.ts`** matches the **skill-issue** CLI (`core` root modules, `commands`, `git`, `registry`). Edit **`ALLOWED_IMPORTS`** and layer detection for other repositories.

## Invocation

```sh
npx tsx registry/validate-deps/hard/index.ts
npm run check:deps
```
...
## Default layer graph (skill-issue)
